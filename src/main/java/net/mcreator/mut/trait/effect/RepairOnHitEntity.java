...
public class RepairOnHitEntity implements TraitEffect {
    private final String trigger;
    private final EntityFilter filter;
    private final int amount;

    public RepairOnHitEntity(String trigger, EntityFilter filter, int amount) {
    ...
    @Override public void apply(LivingEntity user, LivingEntity target, ItemStack stack, BlockState broken) {
        if (target != null && stack.isDamaged() && filter.matches(target)) {
