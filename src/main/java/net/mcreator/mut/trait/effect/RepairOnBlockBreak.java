...
public class RepairOnBlockBreak implements TraitEffect {
    private final String trigger;
    private final BlockFilter filter;
    private final int amount;

    public RepairOnBlockBreak(String trigger, BlockFilter filter, int amount) {
    ...
    @Override public void apply(LivingEntity user, LivingEntity target, ItemStack stack, BlockState broken) {
        if (stack.isDamaged() && broken != null && filter.matches(broken)) {
