...
public class ApplyMobEffectToSelf implements TraitEffect {
    private final String trigger;
    private final Holder<MobEffect> effect;
    private final ValueRange duration, amplifier;
    private final EntityFilter filter;

    public ApplyMobEffectToSelf(String trigger, String effectId, ValueRange duration, ValueRange amplifier, EntityFilter filter) {
    ...
    @Override public void apply(LivingEntity user, LivingEntity target, ItemStack stack, BlockState broken) {
        if (user != null && filter.matches(target != null ? target : user)) {
            ...
            user.addEffect(new MobEffectInstance(effect, duration.rollInt(r), amplifier.rollInt(r)));
        }
    }
}